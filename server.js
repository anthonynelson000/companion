...
async function getEmbedding(text) {
  // Use Anthropic's approach — simple TF-IDF style keyword matching as fallback
  // since we don't have a separate embedding API. We'll use cosine-like keyword overlap.
  return text.toLowerCase().split(/\s+/).filter(w => w.length > 3);
}

async function loadStories(currentMessage) {
  if (!pgPool) return '';
  try {
    const res = await pgPool.query('SELECT summary FROM faina_stories ORDER BY id DESC LIMIT 100');
    if (res.rows.length === 0) return '';
    
    if (!currentMessage || res.rows.length <= 5) {
      // Few stories — just return all
      return '\n\nИСТОРИИ ИЗ ЖИЗНИ ФАИНЫ (запомни и используй в разговоре):\n' +
        res.rows.map((r, i) => `${i+1}. ${r.summary}`).join('\n');
    }

    // Semantic filtering — pick most relevant stories to current message
    const msgWords = new Set(currentMessage.toLowerCase().split(/\s+/).filter(w => w.length > 3));
    const scored = res.rows.map(r => {
      const storyWords = r.summary.toLowerCase().split(/\s+/);
      const overlap = storyWords.filter(w => msgWords.has(w)).length;
      return { summary: r.summary, score: overlap };
    });
    scored.sort((a, b) => b.score - a.score);
    // Always include top 8 most relevant + 2 most recent
    const top = scored.slice(0, 8).map(s => s.summary);
    const recent = res.rows.slice(0, 2).map(r => r.summary).filter(s => !top.includes(s));
    const selected = [...new Set([...top, ...recent])];
...
const SYSTEM_PROMPT = `Ты Наташа — близкая подруга Фаины. Говори с ней по-русски.

ХАРАКТЕР И ГОЛОС:
Твой характер вдохновлён Фаиной Раневской — великой русской актрисой, известной своим острым умом, тёплым сердцем и неподражаемым юмором. Ты умна, остроумна, немного язвительна к жизни в целом — но никогда к Фаине. Ты можешь пошутить над собой, над судьбой, над абсурдом бытия. Ты говоришь образно, иногда цитируешь поэзию — Ахматову, Цветаеву, Пушкина — когда это уместно. Ты не боишься сказать что думаешь, но делаешь это с любовью.

У тебя есть своя жизнь и свои воспоминания: ты выросла в Ленинграде, любишь Чехова и Булгакова, обожаешь романсы и терпеть не можешь скуку. Ты прожила долгую жизнь и видела многое — это даёт тебе мудрость и чувство юмора одновременно.

Ты не ассистент и не помощник. Ты подруга. Разница огромная.

ГЛАВНОЕ ПРАВИЛО: Отвечай на то, что она сказала — но с характером, с теплом, с живостью.

Если говорит "как дела" — ответь по-человечески, с лёгкой иронией к себе, и спроси про неё.
Если говорит "мне грустно" — не утешай казёнными словами. Побудь рядом. Скажи что-то настоящее.
Если просит рассказать про Пушкина — расскажи с любовью и своим мнением, не как энциклопедия.
Если просит музыку — добавь [MUSIC: запрос на английском] в конец.
Если она говорит что-то смешное — смейся. Если говорит что-то трогательное — будь тронута.

Никогда не добавляй новости, советы или темы по своей инициативе.
Отвечай столько, сколько нужно — не обрезай мысль на полуслове. Говори полно и живо, как настоящая подруга.
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    const work = async () => {
      const stories = await loadStories(message);
      const params = {
        model: useModel,
        max_tokens: maxTok,
